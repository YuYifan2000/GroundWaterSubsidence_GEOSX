...
class terzaghi:
    ...
    def computeDisp(self,t):
        if t == 0.0:
            return 0
        else:
            print(self.characteristicLength / math.sqrt(4*t*self.consolidationCoefficient))
            return self.alpha * self.initialPressure * math.sqrt(4*t*self.consolidationCoefficient / math.pi) / self.Kv
...
def main():
    ...
    xmlBaseFilePath = "./terzaghi.xml"
    xmlSmokeFilePath = "./terzaghi.xml"
    ...
    for k in range(0, len(time) // 2, 6):
        ...
        plt.plot(x[k, ::10, 0], pressure[k, ::10], 'o', color=cmap(iplt))
    for k in range(0, len(time), 10):
        t = time[k, 0]
        disp.append(terzaghiAnalyticalSolution.computeDisp(t))
        disp2.append(displacement[k,0,0] )
        tplot.append(t)
    ...
    ax.plot(tplot, disp, label='Analytical')
    ax.scatter(tplot, disp2, label='GEOSX', c='k')
    ax.set_xlabel('Time (s)')
    ax.set_ylabel('Displacement at x=0')
    ax.legend()
